...
function safeExec(command: string, cwd: string, onErrorTag: string, cb?: (err: Error | null) => void) {
  try {
    exec(command, { cwd }, err => {
      if (err) {
        console.warn(`\x1b[33m[${onErrorTag}]\x1b[0m 跳过执行: ${err.message}`);
      }
...
function copy_statusbar_on_done(entry: Entry) {
  return {
    apply(compiler: webpack.Compiler) {
      if (!compiler.options.watch) return;
      const isStatusBar =
        entry.script.includes('黄晓林角色卡') && entry.script.includes('界面') && entry.script.includes('状态栏');
      if (!isStatusBar) return;
      compiler.hooks.done.tap('copy_statusbar_to_regex', () => {
        try {
          exec('node scripts/copy-statusbar-to-regex.js', { cwd: import.meta.dirname }, (_err, stdout, stderr) => {
            if (stdout) process.stdout.write(stdout);
            if (stderr) process.stderr.write(stderr);
          });
        } catch (err: any) {
          console.warn(`\x1b[33m[copy_statusbar]\x1b[0m 跳过执行: ${err.message}`);
        }
      });
    },
  };
